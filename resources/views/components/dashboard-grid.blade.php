<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-4">
    @foreach ($buttons as $button)
        <div class="border border-black p-7 flex justify-center items-center">
            <a href="{{ $button->link ?: route('button.configure', $button->id) }}"
                class="w-1/3 flex justify-center items-center" style="text-decoration: none;">
                <button class="text-white font-bold py-2 px-4 rounded w-full flex justify-center items-center"
                    style="color: {{ $button->color }}; background-color: transparent;">
                    <?xml version="1.0" encoding="iso-8859-1"?>
                    <svg fill="{{ $button->color }}" height="100px" width="100px" version="1.1" id="Capa_1"
